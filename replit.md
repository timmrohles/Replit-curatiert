...
-   **Affiliate Creator Program**: Users can register for the affiliate program via the Dashboard Profile tab ("Geld verdienen mit Empfehlungen" section). Collapsible subsections for identity data, tax info (Kleinunternehmer/Umsatzsteuerpflichtig), payout details (IBAN/BIC), legal consents, and tracking consent. Data stored in `affiliate_creator_profiles` table. API endpoints at `/api/affiliate-creator-profile` (GET/POST).
