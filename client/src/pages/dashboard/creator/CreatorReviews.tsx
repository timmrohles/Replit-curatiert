...
import { MessageSquare, Plus, Edit, Trash2, Eye, EyeOff, Filter, X } from 'lucide-react';
  ...
  const [reviews, setReviews] = useState<Review[]>([
    {
      ...
      content: 'Ein beeindruckendes Werk über die Vergänglichkeit der Zeit...',
      rating: 5,
      status: 'published',
    ...
    },
    {
      ...
      content: 'Eine faszinierende Geschichte über Konformität und Individualität...',
      rating: 4,
      status: 'draft',
    ...
    },
    {
      ...
      content: 'Ein Meisterwerk der deutschen Literatur...',
      rating: 5,
      status: 'published',
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs md:text-sm" style={{ color: '#6B7280' }}>
            Verwalte deine Buchrezensionen
          </p>
        </div>
        <button
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2">
        ...
        >
          Veröffentlicht ({reviews.filter(r => r.status === 'published').length})
        </button>
        <button
        ...
        >
          Entwürfe ({reviews.filter(r => r.status === 'draft').length})
        </button>
      </div>
      ...
      <div className="space-y-3">
        {filteredReviews.length === 0 ? (
          <div className="rounded-lg p-12 text-center border" style={{ backgroundColor: '#FFFFFF', borderColor: '#E5E7EB' }}>
            <MessageSquare className="w-16 h-16 mx-auto mb-4" style={{ color: '#9CA3AF' }} />
            <h3 className="text-xl mb-2" style={{ fontFamily: 'Fjalla One', color: '#3A3A3A' }}>
              Keine Rezensionen
            </h3>
            <p className="text-sm mb-4" style={{ color: '#6B7280' }}>
              {statusFilter === 'all' ? 'Erstelle deine erste Rezension' : `Keine ${statusFilter === 'published' ? 'veröffentlichten' : 'Entwurf-'}Rezensionen`}
            </p>
            {statusFilter === 'all' && (
              <button
                onClick={() => setShowCreateModal(true)}
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-xs" style={{ color: '#9CA3AF' }}>
                    <span>Erstellt: {new Date(review.createdAt).toLocaleDateString('de-DE')}</span>
                    {review.publishedAt && (
                      <span>Veröffentlicht: {new Date(review.publishedAt).toLocaleDateString('de-DE')}</span>
                    )}
                  </div>
                </div>
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl md:text-2xl" style={{ fontFamily: 'Fjalla One', color: '#3A3A3A' }}>
                {editingReview ? 'Rezension bearbeiten' : 'Neue Rezension'}
              </h2>
              <button
                onClick={() => {
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  defaultValue={editingReview?.bookTitle}
              ...
              <div>
                ...
                <input
                  type="text"
                  defaultValue={editingReview?.bookAuthor}
                  className="w-full px-4 py-2 rounded-lg border"
                  style={{ borderColor: '#E5E7EB' }}
